...
def clean_historical_evaluation(model_id, reference_id, runset_id, timestamp, previous_max_time, debug_lvl=0):
    ...
    if max_timestamp is None:
        Debug.dl("evalcln_nashsutcliffe: Failed to get a max timestamp from '{0}'.".format(hist_folder_path), 2,
                 debug_lvl)
        return
    ...
    for cur_hist_filename in hist_file_names:
        ...
        Debug.dl("evalcln_nashsutcliffe: Deleting file {0} (references: {1} and {2}).".format(cur_file_path,
                                                                                              min_timestamp,
                                                                                              max_timestamp),
                 2, debug_lvl)
        count_del_files += 1
    ...
    Debug.dl("evalcln_nashsutcliffe: Deleted {0} files in {1} seconds".format(count_del_files, d_time), 1, debug_lvl)
