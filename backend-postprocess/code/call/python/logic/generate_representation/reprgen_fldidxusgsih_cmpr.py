...
def generate_representation(sc_comparison_id, timestamp, flextime=None, runset_id=None, debug_lvl=0):
    ...
    # read files
    the_data_1 = get_data_vector(sc_model_1, requiered_product, the_timestamp1,
                                 runset_id=runset_id, debug_lvl=debug_lvl)
    the_data_2 = get_data_vector(sc_model_2, requiered_product, the_timestamp2,
                                 runset_id=runset_id, debug_lvl=debug_lvl)

    # basic check
    if the_data_1 is None:
        Debug.dl("reprgen_fldidxusgsih_cmpr: Unable to get a file for '{0}' at {1}.".format(sc_model_1, the_timestamp1),
             1, debug_lvl)
        return
    elif the_data_2 is None:
        Debug.dl("reprgen_fldidxusgsih_cmpr: Unable to get a file for '{0}' at {1}.".format(sc_model_2, the_timestamp2),
             1, debug_lvl)
        return
    ...
    max_link_id = min(min(len(the_data_1), len(the_data_2)), len(thresholds))
    ...
    save_image_file(sc_comparison_id, 'fldidxusgsih', bin_matrix, the_timestamp, runset_id=runset_id,
                    debug_lvl=debug_lvl)

    # debug info
    d_time = time.time() - start_time
    Debug.dl("reprgen_fldidxusgsih_cmpr: generate_representation({0}) function took {1} seconds ".format(sc_comparison_id,
                                                                                                         d_time),
             1, debug_lvl)
