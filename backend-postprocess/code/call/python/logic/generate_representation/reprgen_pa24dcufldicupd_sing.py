...
def generate_representation(sc_model_id, timestamp, flextime=None, runset_id=None, debug_lvl=0):
    ...
    requiered_repres1 = 'preacchil24hh'
    requiered_repres2 = 'dcufldicupd'
    resulting_repres = 'pa24dcufldicupd'
    ...
    if the_timestamp is None:
        Debug.dl("reprgen_pa24dcufldicupd_sing: Not found a matching timestamp for '{0}' and '{1}' ('{2}').".format(
    ...
    img_folder_path = FolderDefinition.get_historical_img_folder_path(sc_model_id, resulting_repres,
                                                                      runset_id=runset_id)
    img_file_name = ImageDefinition.define_historical_file_name(the_timestamp, resulting_repres)
    img_file_path = os.path.join(img_folder_path, img_file_name)
    if os.path.exists(img_file_path):
        Debug.dl("reprgen_pa24dcufldicupd_sing: file already exists '{0}' ".format(img_file_path), 1, debug_lvl)
        return None

    # replacing color
    replace_color = None

    ReprComposition.plot_composition_map(sc_model_id, the_timestamp, resulting_repres, requiered_repres1,
                                         requiered_repres2, replace_color=replace_color, runset_id=runset_id,
                                         debug_lvl=debug_level_arg)

    # debug info
    d_time = time.time() - start_time
    Debug.dl("reprgen_pa24dcufldicupd_sing: generate_representation({0}) function took {1} seconds ".format(sc_model_id,
                                                                                                            d_time),
             1, debug_lvl)
