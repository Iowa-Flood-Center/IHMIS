...
def generate_representation(sc_model_id, timestamp, flextime=None, runset_id=None, debug_lvl=0):
    ...
    if the_timestamp00 is None:
        Debug.dl("reprgen_preacchil06hh: not a single bin file for model {0}.".format(sc_model_id), 1, debug_lvl)
        return False

    # the_timestamp06 = None if the_timestamp00 is None else (the_timestamp00 - (6 * 60 * 60))
    ...
    timestamp_06_filepath = os.path.join(hist_folder_path,
                                         BinDefinition.define_file_name(the_timestamp06, requiered_product))

    # basich check 2
    if not os.path.exists(timestamp_00_filepath):
        Debug.dl("reprgen_preacchil06hh: file 00h not found ({0}).".format(timestamp_00_filepath), 1, debug_lvl)
        return False
    if not os.path.exists(timestamp_06_filepath):
        Debug.dl("reprgen_preacchil06hh: file 06h not found ({0} -> '{1}').".format(the_timestamp00,
                                                                                    timestamp_06_filepath),
            1, debug_lvl)
        return False

    # read data files
    time00_data = np.load(timestamp_00_filepath)
    time06_data = np.load(timestamp_06_filepath)
    ...
    # plot the data
    delta_data = time00_data - time06_data
    bin_matrix = build_data_matrix(mask, delta_data, debug_lvl=debug_lvl)
    save_image_file(sc_model_id, "preacchil06hh", bin_matrix, the_timestamp00, runset_id=runset_id, debug_lvl=debug_lvl)

    # debug info
    d_time = time.time()-start_time
    Debug.dl("reprgen_preacchil06hh: generate_representation({0}) function took {1} seconds ".format(sc_model_id,
                                                                                                     d_time),
             1, debug_lvl)
