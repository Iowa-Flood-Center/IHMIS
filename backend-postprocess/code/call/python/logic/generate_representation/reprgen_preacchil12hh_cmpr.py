...
def generate_representation(sc_comparison_id, timestamp, flextime=None, runset_id=None, debug_lvl=0):
    ...
    if timestamp00_1 is None:
        Debug.dl("reprgen_preacchil12hh_cmpr: not a single bin file for model {0}.".format(sc_model_1), 1, debug_lvl)
        return False
    if timestamp00_2 is None:
        Debug.dl("reprgen_preacchil12hh_cmpr: not a single bin file for model {0}.".format(sc_model_2), 1, debug_lvl)
        return False

    # determine timestamps at time +6 hour
    timestamp12_1 = determine_timestamp12_for_model(sc_model_1, timestamp00_1, requiered_product, runset_id=runset_id,
                                                    debug_lvl=debug_lvl)
    timestamp12_2 = determine_timestamp12_for_model(sc_model_2, timestamp00_2, requiered_product, runset_id=runset_id,
                                                    debug_lvl=debug_lvl)
    ...
    timestamp_12_1_filepath = os.path.join(hist_folder_path_1,
                                           BinDefinition.define_file_name(timestamp12_1, requiered_product))
    timestamp_00_2_filepath = os.path.join(hist_folder_path_2,
                                           BinDefinition.define_file_name(timestamp00_2, requiered_product))
    timestamp_12_2_filepath = os.path.join(hist_folder_path_2,
                                           BinDefinition.define_file_name(timestamp12_2, requiered_product))

    # basich check 2
    if not os.path.exists(timestamp_00_1_filepath):
        Debug.dl("reprgen_preacchil12hh_cmpr: file 00h not found ({0}).".format(timestamp_00_1_filepath), 1, debug_lvl)
        return False
    if not os.path.exists(timestamp_12_1_filepath):
        Debug.dl("reprgen_preacchil12hh_cmpr: file 12h not found ({0} -> '{1}').".format(timestamp00_1,
                                                                                    timestamp_12_1_filepath),
            1, debug_lvl)
        return False
    if not os.path.exists(timestamp_00_2_filepath):
        Debug.dl("reprgen_preacchil12hh_cmpr: file 00h not found ({0}).".format(timestamp_00_2_filepath), 1, debug_lvl)
        return False
    if not os.path.exists(timestamp_12_2_filepath):
        Debug.dl("reprgen_preacchil12hh_cmpr: file 12h not found ({0} -> '{1}').".format(timestamp00_1,
                                                                                    timestamp_12_2_filepath),
            1, debug_lvl)
        return False

    # read data files
    time00_data_1 = np.load(timestamp_00_1_filepath)
    time12_data_1 = np.load(timestamp_12_1_filepath)
    time00_data_2 = np.load(timestamp_00_2_filepath)
    time12_data_2 = np.load(timestamp_12_2_filepath)
    ...
    # plot the data
    delta_data_1 = time00_data_1 - time12_data_1
    delta_data_2 = time00_data_2 - time12_data_2
    delta_data = delta_data_1 - delta_data_2

    bin_matrix = build_data_matrix(mask, delta_data, debug_lvl=debug_lvl)
    save_image_file(sc_comparison_id, "preacchil12hh", bin_matrix, the_timestamp00, runset_id=runset_id,
                    debug_lvl=debug_lvl)

    # debug info
    d_time = time.time()-start_time
    Debug.dl("reprgen_preacchil12hh_cmpr: generate_representation({0}) function took {1} seconds ".format(sc_comparison_id,
                                                                                                          d_time),
             1, debug_lvl)
...
def determine_timestamp00_for_model(sc_model_id, timestamp, flextime, requiered_product, runset_id, debug_lvl=0):
    ...
    if the_timestamp is None:
        Debug.dl("reprgen_preacchil12hh_cmpr: not a single sl bin file for model {0}.".format(sc_model_id), 1, debug_lvl)
        return False

    return the_timestamp


def determine_timestamp12_for_model(sc_model_id, timestamp00, requiered_product, runset_id=None, debug_lvl=0):
    """
