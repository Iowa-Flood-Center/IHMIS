...
def generate_representation(sc_model_id, timestamp_min, timestamp_max, flextime=None, runset_id=None, debug_lvl=0):
    ...
    if None in (the_timestamp_min, the_timestamp_max):
        Debug.dl("reprgen_preacchilh_arbitrary: unable to define a maximum or a minimum timestamp for model...", 1,
                 debug_lvl)
    ...
    if not os.path.exists(timestamp_min_filepath):
        Debug.dl("reprgen_preacchilh_arbitrary: file 00h not found ({0}).".format(timestamp_min_filepath), 1, debug_lvl)
        return False
    if not os.path.exists(timestamp_max_filepath):
        Debug.dl("reprgen_preacchilh_arbitrary: file 24h not found ({0} -> '{1}').".format(the_timestamp_max,
    ...
    Debug.dl("reprgen_preacchilh_arbitrary: Composing with timestamps {0} : {1} -> {2} : {3}).".format(the_timestamp_max,
    ...
    save_image_file(sc_model_id, "preacchilharbitrary", bin_matrix, the_timestamp_max, runset_id=runset_id, debug_lvl=debug_lvl)

    # debug info
    d_time = time.time()-start_time
    Debug.dl("reprgen_preacchil_arbitrary: generate_representation({0}) function took {1} seconds ".format(sc_model_id,
                                                                                                           d_time),
             1, debug_lvl)
