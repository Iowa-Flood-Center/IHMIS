...
def generate_representation(sc_model_id, timestamp, flextime=None, runset_id=None, debug_lvl=0):
    ...
    if the_timestamp00 is None:
        Debug.dl("reprgen_runacchil12hh: not a single bin file for model {0}.".format(sc_model_id), 1, debug_lvl)
        return False
    ...
    if not os.path.exists(timestamp_00_filepath):
        Debug.dl("reprgen_runacchil12hh: file 00h not found ({0}).".format(timestamp_00_filepath), 1, debug_lvl)
        return False
    if not os.path.exists(timestamp_12_filepath):
        Debug.dl("reprgen_runacchil12hh: file 12h not found ({0} -> '{1}').".format(the_timestamp00,
    ...
    save_image_file(sc_model_id, "runacchil12hh", bin_matrix, the_timestamp00, runset_id=runset_id, debug_lvl=debug_lvl)

    # debug info
    d_time = time.time()-start_time
    Debug.dl("reprgen_runacchil12hh: generate_representation({0}) function took {1} seconds ".format(sc_model_id,
                                                                                                     d_time),
             1, debug_lvl)
