...
def generate_representation(sc_comparison_id, timestamp, flextime=None, runset_id=None, debug_lvl=0):
    ...
    if timestamp00_1 is None:
        Debug.dl("reprgen_runacchil24hh_cmpr: not a single bin file for model {0}.".format(sc_model_1), 1, debug_lvl)
        return False
    if timestamp00_2 is None:
        Debug.dl("reprgen_runacchil24hh_cmpr: not a single bin file for model {0}.".format(sc_model_2), 1, debug_lvl)
        return False
    ...
    if not os.path.exists(timestamp_00_1_filepath):
        Debug.dl("reprgen_runacchil24hh_cmpr: file 00h not found ({0}).".format(timestamp_00_1_filepath), 1, debug_lvl)
        return False
    if not os.path.exists(timestamp_24_1_filepath):
        Debug.dl("reprgen_runacchil24hh_cmpr: file 24h not found ({0} -> '{1}').".format(timestamp00_1,
                                                                                         timestamp_24_1_filepath),
            1, debug_lvl)
        return False
    if not os.path.exists(timestamp_00_2_filepath):
        Debug.dl("reprgen_runacchil24hh_cmpr: file 00h not found ({0}).".format(timestamp_00_2_filepath), 1, debug_lvl)
        return False
    ...
    save_image_file(sc_comparison_id, "runacchil24hh", bin_matrix, the_timestamp00, runset_id=runset_id,
                    debug_lvl=debug_lvl)

    # debug info
    d_time = time.time()-start_time
    Debug.dl("reprgen_runacchil24hh_cmpr: generate_representation({0}) function took {1} seconds ".format(sc_comparison_id,
                                                                                                          d_time),
             1, debug_lvl)
