...
def generate_representation(sc_model_id, timestamp, flextime=None, runset_id=None, debug_lvl=0):
    ...
    requiered_product = 'ids_s'  # only sub-surface
    ...
    # basic check
    if the_data is None:
        Debug.dl("reprgen_soilowwacih: Model {0}.{1} does not have a valid set of products.".format(runset_id,
                                                                                                    sc_model_id), 1,
                 debug_lvl)
        return
    ...
    save_image_file(sc_model_id, 'soilowwacih', bin_matrix, the_timestamp, runset_id=runset_id, debug_lvl=debug_lvl)

    # debug info
    d_time = time.time()-start_time
    Debug.dl("reprgen_soilowwacih: generate_representation({0}) function took {1} seconds ".format(sc_model_id, d_time),
             1, debug_lvl)
...
def calculate_by_1(sc_runset_id, sc_model_id, requiered_product, timestamp, flextime=None, debug_lvl=0):
    ...
    hist_st_folder_path = FolderDefinition.get_intermediate_bin_folder_path(sc_model_id, requiered_product,
    ...
    if not os.path.exists(st_filepath):
        Debug.dl("reprgen_soilowwacih: st file not found ({0}).".format(st_filepath), 1, debug_lvl)
        return None, None
