...
class GlobalVar:
    represcomb_id = "hydrographmultiplesalertcurrent"
    days_past = 10
    days_fore = 10
...
def generate_files(modelcomb_id, runset_id, timestamp, timestamp_min, timestamp_max, debug_lvl=0):
    ...
    if (modelcomb_file_path is None) or (not os.path.exists(modelcomb_file_path)):
        Debug.dl("rpcbgen_hydrographmultiplesalertcurrent: File '{0}' not found.".format(modelcomb_file_path), 0, debug_lvl)
        return
    ...
    try:
        ...
        Debug.dl("rpcbgen_hydrographmultiplesalertcurrent: File '{0}' is incomplete.".format(modelcomb_file_path), 0, debug_lvl)
        return

    #
    if represcomb_id not in represcomb_set:
        Debug.dl("rpcbgen_hydrographmultiplesalertcurrent: Modelcomb '{0}.{1}' has no representation comb. '{2}'.".format(
    ...
    for cur_model_id in frame_set.keys():
        ...
        if frame_set[cur_model_id] == "modelpaststg":
            ...
            Debug.dl("rpcbgen_hydrographmultiplesalertcurrent: Unexpected frame '{0}' for model '{1}.{2}'.".format(
        ...
        # if there is alerts, append it to the cumulative variable
...
def generate_modelpaststg_stuff(model_id, runset_id, all_usgs_rc, meta_mng, any_alert=False, timestamp=None,
    ...
    Debug.dl("rpcbgen_hydrographmultiplesalertcurrent: Processing 'modelpaststg' for model '{0}.{1}'.".format(runset_id,
                                                                                                       model_id),
             4, debug_lvl)
    ...
    Debug.dl("rpcbgen_hydrographmultiplesalertcurrent: Reading modeled data from '{0}'.".format(mdl_prod_folder_path), 1,
             debug_lvl)
    ...
    for cur_timestamp in range(the_timestamp_min, the_timestamp_mid, GlobalVar.timestep):
        ...
        if cur_effect_timestamp is None:
            Debug.dl("rpcbgen_hydrographmultiplesalertcurrent: No file for '{0}'.".format(cur_timestamp), 19, debug_lvl)
            continue
        else:
            Debug.dl("rpcbgen_hydrographmultiplesalertcurrent: Considering file '{0}'.".format(cur_prod_file_path), 19,
                     debug_lvl)
    ...
    for cur_link_id in data_dictionary.keys():
        ...
        Debug.dl("rpcbgen_hydrographmultiplesalertcurrent: Wrote file '{0}'.".format(out_file_path), 1, debug_lvl)
    ...
    if stage_thresholds_dict is not None:
        if link_id in stage_thresholds_dict["links"]:
            if stage_thresholds_dict["links"][link_id]["type"] == "elevation":
                if len(data_dictionary[link_id]) == 0:
                    flood_level = 0
                else:
                    last_pair = data_dictionary[link_id][-1]
                    thr_dict = stage_thresholds_dict["links"][link_id]["thresholds"]
                    if (thr_dict["fld_action"] != -1) and (current_flood_level <= 1) and \
                            (last_pair[1] > thr_dict["fld_action"]):
                        flood_level = 1
                    if (thr_dict["fld_flood"] != -1) and (current_flood_level <= 2) and \
                            (last_pair[1] > thr_dict["fld_flood"]):
                        flood_level = 2
                    if (thr_dict["fld_moderate"] != -1) and (current_flood_level <= 3) and \
                            (last_pair[1] > thr_dict["fld_moderate"]):
                        flood_level = 3
                    if (thr_dict["fld_major"] != -1) and (current_flood_level <= 4) and \
                            (last_pair[1] > thr_dict["fld_major"]):
                        flood_level = 4
            else:
                Debug.dl("rpcbgen_hydrographmultiplesalertcurrent: unexpected threshold type '{0}'.".format(
                    stage_thresholds_dict["links"][link_id]["type"]), 1, debug_lvl)
        else:
            Debug.dl("rpcbgen_hydrographmultiplesalertcurrent: link {0} not among thresholds.".format(link_id), 4, debug_lvl)
            flood_level = 0
    else:
...
def generate_modelforestg_stuff(model_id, runset_id, meta_mng, timestamp, timestamp_min, timestamp_max,
    ...
    Debug.dl("rpcbgen_hydrographmultiplesalertcurrent: Processing 'modelforestg' for model '{0}.{1}'.".format(runset_id,
                                                                                                       model_id),
             4, debug_lvl)
    ...
    if effect_timestamp_mid is None:
        Debug.dl("rpcbgen_hydrographmultiplesalertcurrent: not a file close enough to '{0}' for '{1}.{2}.{3}'.".format(
            the_timestamp_mid, runset_id, model_id, prod_id), 1, debug_lvl)
        print("...looking at: {0}".format(mdl_prod_folder_path))
        return added_link_ids, alert_links
    ...
    for cur_link_id in data_dictionary.keys():
        ...
        Debug.dl("rpcbgen_hydrographmultiplesalertcurrent: Writing file '{0}'.".format(out_file_path), 3, debug_lvl)
...
def create_common_file(output_folder_path, link_id, all_stage_thresholds, cur_links_alert, debug_lvl=0):
    ...
    if link_id in all_stage_thresholds["links"].keys():
        ...
        try:
            ...
            Debug.dl("rpcbgen_hydrographmultiplesalertcurrent: Some stage thresholds for link_id {0} is missing.".format(
                link_id), 2, debug_lvl)
    ...
    if link_id in cur_links_alert:
        ...
        Debug.dl("rpcbgen_hydrographmultiplesalertcurrent: Link {0} not in list of alerts.".format(link_id), 2, debug_lvl)
    ...
    Debug.dl("rpcbgen_hydrographmultiplesalertcurrent: Wrote file '{0}'.".format(file_path), 3, debug_lvl)
...
def define_timestamps(timestamp=None, timestamp_min=None, timestamp_max=None, debug_lvl=0):
    ...
    if (timestamp is None) and (timestamp_min is None) and (timestamp_max is None):
        ...
        if not (timestamp_max > timestamp_min):
            Debug.dl("rpcbgen_hydrographmultiplesalertcurrent: invalid timestamps: min ({0}) >= max ({1})'.".format(
        ...
        if not (timestamp_max > timestamp_min):
            ...
            Debug.dl("rpcbgen_hydrographmultiplesalertcurrent: invalid timestamps sequence - not ({0} > {1} > {2})'.".format(
        ...
        Debug.dl("rpcbgen_hydrographmultiplesalertcurrent: unexpected timestamp informations - min:{0}, mid:{1}, max:{2}'.".format(
