...
def import_data(sc_runset_id, sc_model_id='all',  unix_timestamp=None, debug_lvl=0):
    ...
    for cur_sc_model_id in evaluated_sc_models_id:
        ...
        if cur_script_states is not None:
            ...
            Debug.dl("import_data_rt_logic: Running '{0}' for sc model '{1}'".format(sys_call, cur_sc_model_id),
                     2, debug_lvl)
            try:
                Debug.dl("import_data_rt_logic: CALL: '{0}'".format(sys_call),
                         2, debug_lvl)
                os.system(sys_call)
            except OSError:
                Debug.dl("import_data_rt_logic: Failed running '{0}'.".format(sys_call), 1, debug_lvl)
        else:
            Debug.dl("import_data_rt_logic: SC model '{0}' has no script for state binary file generation.".format(
        ...
        if cur_script_hydroforecast is not None:
            sys_call = "{0} -model_sing_id {1} -runset_id {2}".format(cur_script_hydroforecast,
                                                                      cur_sc_model_id, sc_runset_id)
            ...
            Debug.dl(
                "import_data_rt_logic: SC model '{0}' has no related script for hydroforecast binary file generation.".format(
                    cur_sc_model_id), 1, debug_lvl)
    ...
    for cur_sc_reference_id in evaluated_sc_references_id:
        Debug.dl("import_data_rt_logic: Will run it for reference '{0}' at {1}.".format(cur_sc_reference_id,
                                                                                       cur_unix_timestamp),
                 0, debug_lvl)
        ...
        if cur_script is not None:
            sys_call = "{0} -reference_id {1} -runset_id {2}".format(
                cur_script, cur_sc_reference_id, sc_runset_id)
            # sys_call = " ".join([cur_script, cur_sc_reference_id, cur_unix_timestamp, sc_runset_id])
            Debug.dl("import_data_rt_logic: Running '{0}' for sc reference '{1}'".format(sys_call, cur_sc_reference_id),
                     2, debug_lvl)
            try:
                Debug.dl("import_data_rt_logic: CALL: '{0}'".format(sys_call),
                         2, debug_lvl)
