...
def update_display_files(sc_model_id, sc_reference_id, sc_runset_id, timestamp, debug_lvl=0):
    ...
    if timestamp is None:
        ...
        if ref0_timestamp is None:
            Debug.dl("evalupd_disctimeexceeval: No file at '{0}'.".format(eval_folder_path), 2, debug_lvl)
        else:
            Debug.dl("evalupd_disctimeexceeval: Most recent timestamp in '{0}' is {1}.".format(eval_folder_path,
                                                                                               ref0_timestamp),
                     2, debug_lvl)
    else:
...
def update_historical_parameter_representations(sc_model_id, sc_evaluation_id, sc_evaluation, sc_reference_id,
    ...
    # basic check
    if ref0_timestamp is None:
        print("evalupd_disctimeexceeval: Timestamp 0-ref is None. Skipping.")
        return
    ...
    while cur_delta_timestamp < historical_maximum_back_time:
        ...
        if os.path.exists(cur_hist_filepath):
            ...
            Debug.dl("evalupd_disctimeexceeval: Updated '{0}' => '{1}'.".format(os.path.basename(cur_hist_filepath),
                                                                                cur_disp_filepath), 2, debug_lvl)
            count_copied += 1
        else:
            print("evalupd_disctimeexceeval: Not found {0}.".format(cur_hist_filepath))
