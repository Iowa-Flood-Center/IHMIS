...
def update_display_files(modelcomb_id, runset_id, timestamp, debug_lvl=0):
    ...
    if timestamp is None:
        ...
        if ref0_timestamp is None:
            Debug.dl("rpcbupd_bestrms10dayspast: No file at '{0}'.".format(modelpaststg_folder_path), 2, debug_lvl)
        else:
            Debug.dl("rpcbupd_bestrms10dayspast: Most recent timestamp in '{0}' is {1}.".format(
...
def select_most_recent_files(src_folder_path):
    """
    ...
    # obtain_hist_file_linkid(historical_file_name)
    if not os.path.exists(src_folder_path):
        return None
    for cur_file_name in os.listdir(src_folder_path):
...
def update_historical_representations_composition(sc_modelcomb_id, sc_reprcomp_id, sc_runset_id, ref0_timestamp,
    ...
    if not os.path.exists(dest_folder_path):
        ...
        Debug.dl("rpcbupd_bestrms10dayspast: Created folder '{0}'.".format(dest_folder_path), 2, debug_lvl)
    ...
    submit_dict = select_most_recent_files(src_folder_path)
    if submit_dict is None:
        Debug.dl("rpcbupd_bestrms10dayspast: Unable to find the most recent file. Skipping.", 2, debug_lvl)
        return

    # copy all files listed in the dictionary
    count_cpy = 0
    ...
    Debug.dl("rpcbupd_bestrms10dayspast: Deleted previous {0}, copied {1} new files...".format(
