...
def update_display_files(modelcomb_id, runset_id, timestamp, debug_lvl=0):
    ...
    sc_reprcomp = "hydrographmultiples"
    ...
    if timestamp is None:
        ...
        if ref0_timestamp is None:
            Debug.dl("rpcbupd_hydrographmultiples: No file at '{0}'.".format(modelpaststg_folder_path), 2, debug_lvl)
        else:
            Debug.dl("rpcbupd_hydrographmultiples: Most recent timestamp in '{0}' is {1}.".format(modelpaststg_folder_path,
                                                                                                  ref0_timestamp),
                     2, debug_lvl)
...
def get_frame_model_ids(modelcomb_id, runset_id, represcomb_id, frame, debug_lvl=0):
    ...
    if (modelcomb_file_path is None) or (not os.path.exists(modelcomb_file_path)):
        Debug.dl("rpcbupd_hydrographmultiples: File '{0}' not found.".format(modelcomb_file_path), 0, debug_lvl)
        return None
    ...
    try:
        ...
        Debug.dl("rpcbupd_hydrographmultiples: File '{0}' is incomplete.".format(modelcomb_file_path), 0, debug_lvl)
        return None

    #
    if represcomb_id not in represcomb_set:
        Debug.dl("rpcbupd_hydrographmultiples: Modelcomb '{0}.{1}' has no representation comb. '{2}'.".format(
...
def update_historical_representations_composition(sc_modelcomb_id, sc_reprcomp_id, sc_runset_id, ref0_timestamp,
    ...
    # define models
    modelpaststg_model_id = get_frame_model_ids(sc_modelcomb_id, sc_runset_id, sc_reprcomp_id, "modelpaststg",
                                                debug_lvl=debug_lvl)
    modelforestg_model_ids = get_frame_model_ids(sc_modelcomb_id, sc_runset_id, sc_reprcomp_id, "modelforestg",
                                                 debug_lvl=debug_lvl)

    # basic check
    if modelpaststg_model_id == None:
        Debug.dl("rpcbupd_hydrographmultiples: Needs at least one 'modelpaststg' model.", 0, debug_lvl)
        return
    elif not (isinstance(modelpaststg_model_id, str) or isinstance(modelpaststg_model_id, basestring)):
        Debug.dl("rpcbupd_hydrographmultiples: Needs only one 'modelpaststg' model.", 0, debug_lvl)
        return
    elif modelforestg_model_ids is None:
        Debug.dl("rpcbupd_hydrographmultiples: Needs at least one 'modelforestg' model.", 0, debug_lvl)
        return
    ...
    if not os.path.exists(modelpaststg_source_folder_path):
        Debug.dl("rpcbupd_hydrographmultiples: Skipping update - no file found.", 1, debug_lvl)
        return
    ...
    Debug.dl("rpcbupd_hydrographmultiples: Filled '{0}' folder.".format(modelpaststg_dest_folder_path), 1, debug_lvl)
    ...
    for cur_modelforestg_model_id in modelforestg_model_ids:
        ...
        Debug.dl("rpcbupd_hydrographmultiples: Filled '{0}' folder.".format(modelforestg_dest_folder_path), 1, debug_lvl)
    ...
    Debug.dl("rpcbupd_hydrographmultiples: Filled '{0}' folder.".format(common_dest_folder_path), 1, debug_lvl)
