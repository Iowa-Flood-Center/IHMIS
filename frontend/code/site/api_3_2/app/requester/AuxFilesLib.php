<?php
	...
	class AuxFilesLib{
		...
		/**
		 *
		 * TODO - Remove hard coded - replace by DB call
		 * $hillslope_model_id : 
		 * RETURN : Array of Strings with sc_product_ids.
		 */
		public static function get_sc_product_ids($hillslope_model_id){
			$the_model = HlModel::where('id', $hillslope_model_id)->get()->first();
			$all_products = $the_model->scproducts(AuxFilesLib::$app)->get();
			$all_retrieved = array();
			foreach($all_products as $cur_product){
				array_push($all_retrieved, $cur_product['acronym']);
			}
		...
		public static function get_sc_representation_ids($hillslope_model_id){
			$the_model = HlModel::where('id', $hillslope_model_id)->get()->first();
			$all_representations = $the_model->screpresentations(AuxFilesLib::$app)->get();
			$all_retrieved = array();
			foreach($all_representations as $cur_representation){
				array_push($all_retrieved, $cur_representation['acronym']);
			}
			return($all_retrieved);
		}
		...
		/**
		 *
		 * TODO - discontinue it - replace by get_hydrologicalmodel_bin_path()
		 ...
		 * RETURN:
		 */
		public static function get_hydrologicalmodel_bin_path($hl_model_id){
			
			// get the HL-Model and basic check it
			$hl_model = HlModel::where('id', $hl_model_id)->get()->first();
			if(is_null($hl_model)){ return(null); }
			
			// get the HydrologicalModel and basic check it
			$hydrological_model = $hl_model->hydrologicalmodel()->get()->first();
			if(is_null($hydrological_model)){ return(null); }
			
			// return($hl_model['id_hydrologicalmodel']);
			return($hydrological_model['path']);
		}
		 ...
		 * RETURN:
		 */
		public static function get_hydrologicalmodel_path_and_dependencies($hl_model_id){
			// get the HL-Model and basic check it
			$hl_model = HlModel::where('id', $hl_model_id)->get()->first();
			if(is_null($hl_model)){ return(null); }
			...
			// build return array
			$ret_array = array();
			$ret_array["path"] = $hydrological_model['path'];
			$ret_array["dependencies"] = $hydrological_model['dependencies'];
			$ret_array["dependencies"] = str_replace("{", "[", $ret_array["dependencies"]);
			$ret_array["dependencies"] = str_replace("}", "]", $ret_array["dependencies"]);
			$ret_array["dependencies"] = json_decode($ret_array["dependencies"]);
			return($ret_array);
		}
		...
		public static function get_destination_url($hawkid){
			return(AuxFilesLib::DEST_BASE_URL.$hawkid."/");
		}
		...
		public static function get_topology_file_name(){
			return(AuxFilesLib::TOPOLOGY_FILE_NAME);
		}
		...
		/**
		 *
		 * NOTE 1: Inverse method of 'AuxFilesLib::extract_hlmodel_from_initialcondition_file_name()'.
		 * NOTE 2: Inverse method of 'AuxFilesLib::extract_timestamp_from_initialcondition_file_name()'.
		 * $hillslope_model_id :
		 * $initcond_timestamp :
		 * RETURN : String
		 */
		public static function get_initialcondition_file_name($hillslope_model_id, $initcond_timestamp){
			return("state".$hillslope_model_id."_".$initcond_timestamp.AuxFilesLib::INITCOND_FILE_EXT);
		}
		...
		*/
		...
		/**
		 *
		 * NOTE : Inverse method of 'AuxFilesLib::get_initialcondition_file_name()'.
		...
		public static function extract_hlmodel_from_initialcondition_file_name($initcond_filename){
			...
			// process string
			$basename = str_replace(AuxFilesLib::INITCOND_FILE_EXT, "", $initcond_filename);
			$basename = str_replace(AuxFilesLib::INITCOND_FILE_PREFIX, "", $basename);
		...
		public static function get_topology_template_file_path(){
			$topo_file_name = AuxFilesLib::get_topology_file_name();
			return(AuxFilesLib::$app->fss->template_files_folder_path.$topo_file_name);
		}
		...
		public static function get_demparameters_template_file_path($hillslope_model_id){
			...
			return(AuxFilesLib::$app->fss->template_files_folder_path.$demparameters_file_name);
		}
		...
		public static function get_initcond_repo_file_path($hillslope_model_id, $initcond_timestamp){
			$initcond_filename = AuxFilesLib::get_initialcondition_file_name($hillslope_model_id, $initcond_timestamp);
		...
		public static function get_raindata_template_file_path($precipitation_source_id){
			...
			return(AuxFilesLib::$app->fss->template_files_folder_path.$forcingprecip_file_name);
		}
		...
		public static function get_evaporation_template_file_path(){
			...
			return(AuxFilesLib::$app->fss->template_files_folder_path.$evapotranspiration_file_name);
		}
		...
		public static function get_qvs_template_file_path(){
			return(AuxFilesLib::$app->fss->template_files_folder_path . AuxFilesLib::get_qvs_file_name());
		}
		...
		public static function get_rsvdbc_template_file_path(){
			return(AuxFilesLib::$app->fss->template_files_folder_path . AuxFilesLib::get_rsvdbc_file_name());
		}
		...
		public static function get_rsv_template_file_path(){
			return(AuxFilesLib::$app->fss->template_files_folder_path . AuxFilesLib::get_rsv_file_name());
		}
		...
		public static function get_local_folder_path($hawk_id){
			return(AuxFilesLib::$app->fss->temp_folderpath.$hawk_id."/");
		}
		...
		public static function get_local_temp_folder_path($current_timestamp){
			return(AuxFilesLib::$app->fss->temp_folderpath.$current_timestamp."/");
		}
		...
		public static function get_local_topology_file_path($hawk_id){
			return(AuxFilesLib::get_local_folder_path($hawk_id).AuxFilesLib::TOPOLOGY_FILE_NAME);
		}
		...
		public static function get_local_temp_topology_file_path($current_timestamp){
			return(AuxFilesLib::get_local_temp_folder_path($current_timestamp).AuxFilesLib::TOPOLOGY_FILE_NAME);
		}
		...
		public static function get_local_temp_meta_folder_path($current_timestamp, $subfolder=false){
			if(!$subfolder){
				//return(AuxFilesLib::get_local_temp_folder_path($current_timestamp).AuxFilesLib::METAFILES_FOLDER_NAME."/");
				return(AuxFilesLib::get_local_temp_folder_path($current_timestamp).AuxFilesLib::METAFILES_FOLDER_NAME."/");
				
			} else {
				if ($subfolder == "models"){
					return(AuxFilesLib::get_local_temp_folder_path($current_timestamp).AuxFilesLib::METAFILES_FOLDER_NAME."/".AuxFilesLib::METAFILES_MODELS_FOLDER_NAME."/");
				} elseif ($subfolder == "matrices") {
					return(AuxFilesLib::get_local_temp_folder_path($current_timestamp).AuxFilesLib::METAFILES_FOLDER_NAME."/".AuxFilesLib::METAFILES_MATRICES_FOLDER_NAME."/");
				} elseif ($subfolder == "runset") {
					return(AuxFilesLib::get_local_temp_folder_path($current_timestamp).AuxFilesLib::METAFILES_FOLDER_NAME."/".AuxFilesLib::METAFILES_RUNSET_FOLDER_NAME."/");
				} elseif ($subfolder == "modelcomb"){
					return(AuxFilesLib::get_local_temp_folder_path($current_timestamp).AuxFilesLib::METAFILES_FOLDER_NAME."/".AuxFilesLib::METAFILES_MODELSCOMB_FOLDER_NAME."/");
				} else {
					return(null);
				}
			}
		...
		public static function get_local_dempars_file_path($hawk_id, $hillslope_model_id){
			return(AuxFilesLib::get_local_folder_path($hawk_id).AuxFilesLib::get_dempars_file_name($hillslope_model_id));
		}
		...
		public static function get_local_temp_dempars_file_path($current_timestamp, $hillslope_model_id){
			return(AuxFilesLib::get_local_folder_path($current_timestamp).AuxFilesLib::get_dempars_file_name($hillslope_model_id));
		}
		...
		public static function get_local_initcondition_file_path($hawk_id, $hillslope_model_id, $initcond_timestamp){
			$initcond_filename = AuxFilesLib::get_initialcondition_file_name($hillslope_model_id, $initcond_timestamp);
			return(AuxFilesLib::get_local_folder_path($hawk_id).$initcond_filename);
		}
		...
		public static function get_local_temp_initcondition_file_path($current_timestamp, $hillslope_model_id, $initcond_timestamp){
			$initcond_filename = AuxFilesLib::get_initialcondition_file_name($hillslope_model_id, $initcond_timestamp);
			return(AuxFilesLib::get_local_temp_folder_path($current_timestamp).$initcond_filename);
		}
		...
		public static function get_local_raindata_file_path($hawk_id, $precipitation_source_id){
			$raindata_filename = AuxFilesLib::get_raindata_file_name($precipitation_source_id);
			return(AuxFilesLib::get_local_folder_path($hawk_id).$raindata_filename);
		}
		...
		public static function get_local_temp_raindata_file_path($current_timestamp, $precipitation_source_id){
			$raindata_filename = AuxFilesLib::get_raindata_file_name($precipitation_source_id);
			return(AuxFilesLib::get_local_temp_folder_path($current_timestamp).$raindata_filename);
		}
		...
		public static function get_local_qvs_file_path($hawkid){
			return(AuxFilesLib::get_local_folder_path($hawkid).AuxFilesLib::get_qvs_file_name());
		}
		...
		public static function get_local_temp_qvs_file_path($current_timestamp){
			return(AuxFilesLib::get_local_temp_folder_path($current_timestamp).AuxFilesLib::get_qvs_file_name());
		}
		...
		public static function get_local_temp_rsvdbc_file_path($current_timestamp){
			return(AuxFilesLib::get_local_temp_folder_path($current_timestamp).AuxFilesLib::get_rsvdbc_file_name());
		}
		...
		public static function get_local_temp_rsv_file_path($current_timestamp){
			return(AuxFilesLib::get_local_temp_folder_path($current_timestamp).AuxFilesLib::get_rsv_file_name());
		}
		...
		public static function get_local_evaporation_file_path($hawkid){
			return(AuxFilesLib::get_local_folder_path($hawkid).AuxFilesLib::get_evaporation_file_name());
		}
		...
		public static function get_local_temp_evaporation_file_path($current_timestamp){
			return(AuxFilesLib::get_local_temp_folder_path($current_timestamp).AuxFilesLib::get_evaporation_file_name());
		}
		...
		public static function get_local_temp_targz_file_path($current_timestamp){
			return("/tmp/".AuxFilesLib::get_input_targz_file_name($current_timestamp));
		}
		...
		public static function get_local_metamodel_file_path($current_timestamp, $model_id){
			return(AuxFilesLib::get_local_temp_meta_folder_path($current_timestamp, "models").$model_id.".json");
		}
		...
		public static function get_local_metacomparisonmtx_file_path($current_timestamp){
			return(AuxFilesLib::get_local_temp_meta_folder_path($current_timestamp, "matrices")."Comparison_matrix.json");
		}
		...
		public static function get_local_metaevaluationmtx_file_path($current_timestamp){
			return(AuxFilesLib::get_local_temp_meta_folder_path($current_timestamp, "matrices")."Evaluation_matrix.json");
		}
		...
		public static function get_local_metarunset_file_path($current_timestamp){
			return(AuxFilesLib::get_local_temp_meta_folder_path($current_timestamp, "runset")."Runset.json");
		}
		...
		public static function get_local_metacomb_hydrographpast_file_path($current_timestamp, $modelcomb_id){
			if (is_null($modelcomb_id)){
				return(AuxFilesLib::get_local_temp_meta_folder_path($current_timestamp, "modelcomb")."hydrogcomb.json");
			} else {
				return(AuxFilesLib::get_local_temp_meta_folder_path($current_timestamp, "modelcomb").$modelcomb_id.".json");
			}
		}
		...
		public static function get_local_temp_output_snapshot_folder_path($current_timestamp){
			return(AuxFilesLib::get_local_temp_folder_path($current_timestamp)."outputs/");
		}
		...
		public static function get_local_temp_output_snapshot_specific_folder_path($current_timestamp, $model_id){
			return(AuxFilesLib::get_local_temp_output_snapshot_folder_path($current_timestamp).$model_id."/");
		}
		...
		public static function get_local_runset_waitingroom_folder_path(){
			return(AuxFilesLib::$app->fss->waiting_room_folder_path);
		}
		...
		public static function get_local_runset_waitingroom_targz_file_path($current_timestamp){
			return(AuxFilesLib::$app->fss->waiting_room_folder_path . AuxFilesLib::get_input_targz_file_name($current_timestamp));
		}
		...
		public static function get_remote_central_folder_path(){
			...
			return(AuxFilesLib::$app->fss->dest_central_folder_path);
		}
		...
		public static function get_remote_folder_path($current_timestamp){
			...
			return(AuxFilesLib::$app->fss->dest_folder_path.$current_timestamp."/" );
		}
		...
		public static function get_remote_output_snapshot_folder_path($current_timestamp, $model_id){
			return(AuxFilesLib::$app->fss->dest_folder_path.$current_timestamp."/outputs/".$model_id."/");
		}
		...
		public static function get_remote_demparameters_file_path($current_timestamp, 
			...
			$dempar_file_path = AuxFilesLib::get_dempars_file_name($hillslope_model_id);
		...
		public static function get_remote_initcond_raw_file_path($current_timestamp, $hillslope_model_id, $initcond_timestamp, $asynch_version){
			$remote_folder_path = AuxFilesLib::get_remote_initialcondition_repo_folder_path($hillslope_model_id, 
			                                                                             $asynch_version);
			$initcond_file_name = AuxFilesLib::get_initialcondition_file_name($hillslope_model_id, 
			                                                               $initcond_timestamp);
		...
		public static function get_remote_initcond_mod_file_path($current_timestamp, $hillslope_model_id, $initcond_timestamp, $model_id, $asynch_version){
			// $remote_folder_path = AuxFilesLib::get_remote_folder_path($current_timestamp);
			// $initcond_file_name = AuxFilesLib::get_initialcondition_file_name($hillslope_model_id, $initcond_timestamp);  // TODO - check if it works
			
			// define source model id
			if(strpos($model_id, 'past') !== false){
				// if this is 'past', get from 'prev'
				$source_model_id = str_replace("past", "prev", $model_id);
			
			} elseif(strpos($model_id, 'fore') !== false){
				// if this is 'fore', get from 'past'
				...
				// if this is 'prev', get from archive
				return(AuxFilesLib::get_remote_initcond_raw_file_path($current_timestamp, $hillslope_model_id, $initcond_timestamp, $asynch_version));
			
			} else {
				// if none of these, get from 'prevqpe'
				$source_model_id = $model_id."prevqpe";
			}
			
			return(AuxFilesLib::get_remote_output_snapshot_file_path($source_model_id, $current_timestamp, $initcond_timestamp, 
																     $asynch_version, true));
		}
		...
		public static function get_remote_raindata_file_path($current_timestamp, $precipitation_source_id){
			$remote_folder_path = AuxFilesLib::get_remote_folder_path($current_timestamp);
			$precipdata_file_name = AuxFilesLib::get_raindata_file_name($precipitation_source_id);
		...
		public static function get_remote_qvs_file_path($current_timestamp){
			return(AuxFilesLib::get_remote_folder_path($current_timestamp).AuxFilesLib::get_qvs_file_name());
		}
		...
		public static function get_remote_output_snapshot_file_path($model_id, $current_timestamp, $init_timestamp, $asynch_version, $force_snapshot){
			$snapshot_file_name = AuxFilesLib::get_output_snapshot_file_name($init_timestamp, $asynch_version, $force_snapshot);
			if (!is_null($snapshot_file_name)){
				$temp_folder_path = AuxFilesLib::get_remote_output_snapshot_folder_path($current_timestamp, $model_id);
		...
		public static function get_remote_scratch_file_path($current_timestamp){
			return(AuxFilesLib::get_remote_folder_path($current_timestamp)."tmp");
		}
		...
		public static function get_remote_global_file_path($current_timestamp, $model_id){
			return(AuxFilesLib::get_remote_folder_path($current_timestamp).$model_id.".gbl");
		}
		...
		public static function get_remote_metamodel_file_path($current_timestamp, $model_id){
			return(AuxFilesLib::get_remote_folder_path($current_timestamp).$model_id.".json");
		}
		...
		public static function setup_local_topology_file($hawk_id){
			...
			$local_topology_file_path = AuxFilesLib::get_local_topology_file_path($hawk_id);
			if(file_exists($local_topology_file_path)){
				return;
			}
			// file does not exist. Create it.
			$template_topology_file_path = AuxFilesLib::get_topology_template_file_path();
		...
		public static function setup_local_temp_topology_file($current_timestamp){
			// check if file already exists. If exists, go home
			$local_topology_file_path = AuxFilesLib::get_local_temp_topology_file_path($current_timestamp);
			if(file_exists($local_topology_file_path)){
				return;
			}
			// file does not exist. Create it.
		...
		public static function setup_local_demparameters_file($hawkid, $hillslope_model_id){
			// check if file already exists. If exists, go home
			$local_demparameters_file_path = AuxFilesLib::get_local_dempars_file_path($hawkid, $hillslope_model_id);
			if(file_exists($local_demparameters_file_path)){
				return;
			}
			// file does not exist. Create it.
			$template_demparameters_file_path = AuxFilesLib::get_demparameters_template_file_path($hillslope_model_id);
		...
		public static function setup_local_temp_demparameters_file($current_timestamp, $hillslope_model_id){
			// check if file already exists. If exists, go home
			$local_demparameters_file_path = AuxFilesLib::get_local_temp_dempars_file_path($current_timestamp, $hillslope_model_id);
			if(file_exists($local_demparameters_file_path)){
				return;
			}
			// file does not exist. Create it.
		...
		public static function setup_local_initcond_file($hawkid, $hillslope_model_id, $initcond_timestamp){
			...
			$initcond_temp_filepath = AuxFilesLib::get_local_initcondition_file_path($hawkid, $hillslope_model_id, $initcond_timestamp);
			if(file_exists($initcond_temp_filepath)){
				return(True);
			}
			
			// define source file and check it
			$initcond_repo_filepath = AuxFilesLib::get_initcond_repo_file_path($hillslope_model_id, $initcond_timestamp);
		...
		*/
		...
		public static function setup_local_temp_initcond_file($current_timestamp, $hillslope_model_id, $initcond_timestamp){
			...
			// define destination file and check if it already exists
			$initcond_temp_filepath = AuxFilesLib::get_local_temp_initcondition_file_path($current_timestamp, $hillslope_model_id, $initcond_timestamp);
			if(file_exists($initcond_temp_filepath)){
				return(True);
			}
		...
		*/
		...
		public static function setup_local_raindata_file($hawkid, $precipitation_source_id){
			$raindata_template_filepath = AuxFilesLib::get_raindata_template_file_path($precipitation_source_id);
			$raindata_temp_filepath = AuxFilesLib::get_local_raindata_file_path($hawkid, $precipitation_source_id);
		...
		public static function setup_local_temp_raindata_file($current_timestamp, $precipitation_source_id){
			$raindata_template_filepath = AuxFilesLib::get_raindata_template_file_path($precipitation_source_id);
			$raindata_temp_filepath = AuxFilesLib::get_local_temp_raindata_file_path($current_timestamp, $precipitation_source_id);
			if(!file_exists($raindata_temp_filepath)){
				copy($raindata_template_filepath, $raindata_temp_filepath);
			}
		}
		...
		public static function setup_local_evaporation_file($hawkid){
			$evapot_template_filepath = AuxFilesLib::get_evaporation_template_file_path();
			$evapot_temp_filepath = AuxFilesLib::get_local_evaporation_file_path($hawkid);
		...
		public static function setup_local_temp_evaporation_file($current_timestamp){
			$evapot_template_filepath = AuxFilesLib::get_evaporation_template_file_path();
			$evapot_temp_filepath = AuxFilesLib::get_local_temp_evaporation_file_path($current_timestamp);
			if(!file_exists($evapot_temp_filepath)){
				copy($evapot_template_filepath, $evapot_temp_filepath);
			}
		}
		...
		public static function setup_local_qvs_file($hawkid){
			$qvs_template_filepath = AuxFilesLib::get_qvs_template_file_path();
			$qvs_temp_filepath = AuxFilesLib::get_local_qvs_file_path($hawkid);
		...
		public static function setup_local_temp_qvs_file($current_timestamp){
			$qvs_template_filepath = AuxFilesLib::get_qvs_template_file_path();
			$qvs_temp_filepath = AuxFilesLib::get_local_temp_qvs_file_path($current_timestamp);
			if(!file_exists($qvs_temp_filepath)){
				copy($qvs_template_filepath, $qvs_temp_filepath);
			}
		}
		...
		public static function setup_local_temp_reservoir_files($current_timestamp){
			// copy dbc file
			$dbc_template_filepath = AuxFilesLib::get_rsvdbc_template_file_path();
			$dbc_temp_filepath = AuxFilesLib::get_local_temp_rsvdbc_file_path($current_timestamp);
			if(!file_exists($dbc_temp_filepath)){
				copy($dbc_template_filepath, $dbc_temp_filepath);
			}
			
			// copy rsv file
			$rsv_template_filepath = AuxFilesLib::get_rsv_template_file_path();
			$rsv_temp_filepath = AuxFilesLib::get_local_temp_rsv_file_path($current_timestamp);
