<?php
	...
	class GlobalFileRequest{
		...
		private function create_gbl_file($runset_request_obj, $app){
			...
			$gbl_final_content[26] = "0 ".AuxFilesLib::get_remote_topology_file_path($current_timestamp)."\n";        // replace topology source
			$gbl_final_content[29] = "0 ".AuxFilesLib::get_remote_demparameters_file_path($this->timestamp_cur, 
																						  $this->hillslope_model_id);   // replace DEM parameters
			$gbl_final_content[29] .= "\n";
			$gbl_final_content[33] = "4 ".AuxFilesLib::get_remote_initcond_mod_file_path($this->timestamp_cur, 
																					  $this->hillslope_model_id, 
																					  $this->timestamp_ini, 
																					  $this->model_id, 
																					  $asynch_version)."\n";
			
			$gbl_final_content[37] = str_replace("<FORCINGS_NUMBER>", 
			                                     sizeof($this->forcings_dict), 
												 $gbl_final_content[37]);                                            // replace forcings number
			$is_first = true;
			$gbl_final_content[42] = "";
			foreach($this->forcings_dict as $cur_idx => $cur_forcing_source){
				...
				if($is_first){
					$gbl_final_content[39] = $cur_forcing_lines[0]."\n";
					$gbl_final_content[40] = $cur_forcing_lines[1]."\n";
					$gbl_final_content[41] = $cur_forcing_lines[2]."\n";
				} else {
					$gbl_final_content[42] .= "\n".$cur_forcing_lines[0];
					$gbl_final_content[42] .= "\n".$cur_forcing_lines[1];
					$gbl_final_content[42] .= "\n".$cur_forcing_lines[2];
			...
			if(trim($gbl_final_content[42]) != ""){ $gbl_final_content[42] .= "\n";}
			
			$gbl_final_content[46] = "3 ".AuxFilesLib::get_remote_qvs_file_path($this->timestamp_cur)."\n";             // replace QVS
			$gbl_final_content[49] = $this->define_reservoir_links_line($app);                                          // replace reservoirs reference
			if (!is_null($this->initcond_source)) {
				$gbl_final_content[69] = "4 60 ".AuxFilesLib::get_remote_output_snapshot_file_path($this->model_id, 
																								$this->timestamp_cur, 
																								$this->timestamp_ini,
																								$asynch_version,
																								false)."\n";   // replace output snapshot
			} else {
				$gbl_final_content[69] = "3 ".AuxFilesLib::get_remote_output_snapshot_file_path($this->model_id, 
																							 $this->timestamp_cur, 
																							 $this->timestamp_end,
																							 $asynch_version,
																							 true)."\n";   // replace output snapshot
			}
			$gbl_final_content[72] = AuxFilesLib::get_remote_scratch_file_path($this->timestamp_cur)."\n";              // 
