<?php
  ...
  $sc_represcomb_id = "hydrographmultiplesdotraw";
  ...
  $common_folder_url = $root_folder_url."common/";
  $modelforestg_folder_url = $root_folder_url."modelforestg/";
  $modelpaststg_folder_url = $root_folder_url."modelpaststg/";
  $modelforestgalert_folder_url = $root_folder_url."modelforestgalert/";
  ...
  $all_scmodel_folders = DataAccess::list_datafolder_content($modelforestg_folder_url, 
                                                             $sc_runset_id, 
                                                             "\/");
  ...
  foreach($all_scmodel_folders as $cur_scmodel_folder){
    ...
    $cur_subfolder_url = $modelforestg_folder_url.$cur_scmodel_folder."/";
    $all_inner_files = DataAccess::list_datafolder_content($cur_subfolder_url, $sc_runset_id, ".json");
    # debug check
    foreach($all_inner_files as $cur_inner_file){
      if (check_file_is_linkid($cur_inner_file, $link_id)){
        $cur_inner_file_url = $cur_subfolder_url.$cur_inner_file;
        $cur_file_content = DataAccess::get_datafile_content($cur_inner_file_url, $sc_runset_id);
        $modelforestg_dict[$cur_scmodel_folder] = json_decode($cur_file_content, true);
      }
    }
  }
  
  // find respective forecast alert files
  $all_scmodel_folders = DataAccess::list_datafolder_content($modelforestgalert_folder_url, 
                                                             $sc_runset_id, 
                                                             "\/");
  foreach($all_scmodel_folders as $cur_scmodel_folder){
    // ignore back folders references
  ...
  $all_scmodel_folders = DataAccess::list_datafolder_content($modelpaststg_folder_url, 
                                                             $sc_runset_id, 
                                                             "\/");
  foreach($all_scmodel_folders as $cur_scmodel_folder){
    // ignore back folders references
    ...
    $cur_subfolder_url = $modelpaststg_folder_url.$cur_scmodel_folder."/";
    $all_inner_files = DataAccess::list_datafolder_content($cur_subfolder_url, $sc_runset_id, ".json");
    
    foreach($all_inner_files as $cur_inner_file){
      if (check_file_is_linkid($cur_inner_file, $link_id)){
        $cur_inner_file_url = $cur_subfolder_url.$cur_inner_file;
        $modelpaststg_dict[$cur_scmodel_folder] = json_decode(DataAccess::get_datafile_content($cur_inner_file_url, $sc_runset_id), true);
      }
    }
  }
  
  // load common information
  $common_file_url = $common_folder_url.$link_id.".json";
  $common_dict = json_decode(DataAccess::get_datafile_content($common_file_url, $sc_runset_id), true);
