<?php
  ...
  // read models_only
  if(isset($_GET['sc_models_only']) && (strlen(trim($_GET['sc_models_only'])) > 0)){
    $sc_models_only = explode(",", $_GET['sc_models_only']);
  } else {
    $sc_models_only = null;
  }
  ...
  $meta_model_file_frame = "sc_models/%s.json";   // % runset model
  $root_folder_frame = "repres_displayed/%s/%s/";
  $root_folder_path = sprintf($root_folder_frame, $sc_model_id, 
                              $sc_represcomb_id);
   ...
   */
    ...
    try{
      $file_path = sprintf($meta_model_file_frame, $model_id);
      $file_content = DataAccess::get_metafile_content($file_path, 
                                                       $runset_id);
  ...
  function fill_resume_with_forecast(&$resume_obj, $modelforestg_folder_path, $lead_timestamp, 
    ...
    foreach($model_ids as $cur_model_id){
      ...
      $last_timestamp_before = null;
      $last_scanned_file = null;
      
      // select the best fitting file
      foreach($scanned_files as $cur_scanned_file){
        ...
        if ($cur_timestamp < $min_timestamp){ 
          $last_timestamp_before = $cur_timestamp;
          $last_scanned_file = $cur_scanned_file;
          continue; 
        } elseif ($cur_timestamp > $max_timestamp) {
          break;
        }
      }
      
      // add the best fitting file to the graph
      if ($last_timestamp_before != null){
        
        $cur_leadtime = $last_timestamp_before;
        $cur_file_path = $cur_folder_path.$last_scanned_file;
   ...
   * No return. Changes resume object.
   */
  function fill_resume_with_observed(&$resume_obj, $ref_folder_path, 
                                     $cur_timestamp, $link_id, 
                                     $is_refresh, $sc_runset_id){
   ...
   * No return. Changes resume object.
   */
  function fill_resume_with_commons(&$resume_obj, $common_folder_path, 
                                    $link_id, $is_refresh, 
                                    $is_snapshot, $sc_runset_id){
    if($is_refresh){ return; }
   ...
   */
  function define_current_timestamp($ref_folder_path, $link_id, $sc_runset_id){
    $scanned_files = DataAccess::list_datafolder_content($ref_folder_path,
                                                         $sc_runset_id,
                                                         ".json");
  ...
  function define_stagefore_models($stgfore_folder_path, 
                                   $restrict_models_id, 
                                   $sc_runset_id){
    $scanned_files = DataAccess::list_datafolder_content($stgfore_folder_path,
                                                         $sc_runset_id,
                                                         "\/");
    $model_ids = [];
    foreach($scanned_files as $cur_scanned_file){
      if ((!is_null($restrict_models_id)) && (!in_array($cur_scanned_file, 
                                                        $restrict_models_id ))){
        continue; 
      }
  ...
  function define_stageref_reference($stgref_folder_path, $link_id, 
                                     $sc_runset_id){
    $scanned_folders = DataAccess::list_datafolder_content($stgref_folder_path,
                                                           $sc_runset_id,
                                                           "\/");
  ...
  fill_resume_with_commons($resume_obj, $common_folder_path, $link_id, 
                           $is_refresh, $is_snapshot, $sc_runset_id);
  
  // 5 - fill with observed data
  fill_resume_with_observed($resume_obj, $ref_folder_path, 
                            $timestamp_cur, $link_id, 
                            $is_refresh, $sc_runset_id);
  
  // 6 - fill with forecast data
  fill_resume_with_forecast($resume_obj, $modelforestg_folder_path, $lead_timestamp, $timestamp_cur, 
                            $link_id, $sc_runset_id, $is_refresh, $sc_models_only);
