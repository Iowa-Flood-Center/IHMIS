function custom_display(){
	...
	reprcomp_id = "hydrographmultiplesalert";
	all_links_dict = null;
	...
	console.log("hydrographmultiplesalert: for "+runset_id+"."+modelcomb_id);
	...
	console.log("hydrographmultiplesalert: calling '"+ws_data_url+"'.");
	$.ajax({
		url: ws_data_url
	...
	// load all locations
	console.log("hydrographmultiplesalert: calling '"+ws_gages_location_url+"'.");
	$.ajax({
	...
	function display_when_possible(){
		...
		for(idx=0; idx<json_gage.length; idx++){
			...
			console.log("hydrographmultiplesalert: For link " + cur_linkid + ", created '" + cur_icon_address + "'.");
							...
							for(var k in json_data["past"]){
								...
								if(delta_glue_dict != null){
									if(!(k in delta_glue_dict)){
										console.log("hydrographmultiplesalert: Delta glue not found for '" + k + "'.");
										cur_delta_glue = 0;
									} else {
