<?php
	...
	function fill_resume_with_forecast(&$resume_obj, $modelforestg_folder_path, $lead_timestamp, 
		...
		// find models
		$model_ids = define_stagefore_models($modelforestg_folder_path, $restrict_models_id);
	 ...
	 */
	function define_stagefore_models($stgfore_folder_path, $restrict_models_id){
		...
		foreach($scanned_files as $cur_scanned_file){
			...
			if (is_dir($cur_folder_path)){ 
				if ((!is_null($restrict_models_id)) && (!in_array($cur_scanned_file, $restrict_models_id ))){
					continue; }
